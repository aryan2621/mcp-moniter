...
function MetricsBody() {
  ...
  const { metrics, pagination, isLoading, error } = useMetrics(id, { ...filters, page, limit: pageSize })
  ...
  const handleExport = () => {
    if (!metrics) return
    const csv = [
      ['Timestamp', 'Tool', 'Duration', 'Input Size', 'Output Size', 'Success', 'Error']
        .map(csvEscape)
        .join(','),
      ...metrics.map((m) =>
        [m.timestamp, m.toolName, m.duration, m.inputSize, m.outputSize, m.success, m.error || '']
          .map(csvEscape)
          .join(',')
      ),
    ].join('\n')
    ...
    <div className="space-y-4">
      <div className="flex flex-nowrap items-center justify-between gap-3 pt-2">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <Button size="icon" variant="outline" onClick={() => setFilterOpen(true)} aria-label="Filters">
      ...
      <Sheet open={filterOpen} onOpenChange={setFilterOpen}>
        <SheetContent side="right">
          ...
          <div className="mt-6">
            <MetricsFilters variant="drawer" onFilter={handleFilter} onClose={() => setFilterOpen(false)} />
          </div>
        </SheetContent>
      </Sheet>
